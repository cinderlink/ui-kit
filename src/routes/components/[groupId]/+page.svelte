	...
	{#if group}
		...
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
			{#each stories as { story }}
				<Card href="/components/{story.group}/{story.id}" title={story.title} align="items-start" class="transition-all duration-300 hover:shadow-lg hover:scale-102">
					<Typography el="p" class="text-sm text-gray-800 dark:text-gray-200 leading-relaxed">{story.description}</Typography>
				</Card>
			{/each}
		</div>
	{:else}
		<div class="error-panel">
			<Typography el="h2" class="text-red-900 dark:text-red-100 mb-4 font-semibold">404 - Component Group Not Found</Typography>
			<p class="text-red-800 dark:text-red-200 mb-6">Component group not found: <strong>{$page.params.groupId}</strong></p>
