  ...
  test('Generate contrast report', async ({ page }) => {
    ...
    for (const theme of ['light', 'dark']) {
      ...
      const colors = await page.evaluate(() => {
        ...
        return {
          ...
          sidebarText: sidebar?.querySelector('a') ? 
            window.getComputedStyle(sidebar.querySelector('a')!).color : 'N/A',
      ...
      if (colors.sidebarBg && colors.sidebarText && colors.sidebarBg !== 'N/A') {
        report[theme].sidebarContrast = getContrastRatio(colors.sidebarText, colors.sidebarBg).toFixed(2);
      }
      if (colors.buttonBg && colors.buttonText && colors.buttonBg !== 'N/A') {
        report[theme].buttonContrast = getContrastRatio(colors.buttonText, colors.buttonBg).toFixed(2);
      }
    }
